...
test_that("texenv works with all args", {
...
test_that("texenv throws error with an invalid LaTeX macro name", {
  expect_error(texenv("a1", ""), regexp = "valid LaTeX")
})

test_that("texenv throws error with non-character optargs", {
  expect_error(texenv("foo", "", optargs = TRUE),
               regexp = "not a character vector")
})

test_that("as.character.texenv works with only required args", {
  x <- texenv("foo", "bar")
  expect_equal(as.character(x),
               "\\begin{foo}bar\\end{foo}")
})

test_that("as.character.texenv works with all args", {
...
test_that("begin function works", {
  expect_equal(as.character(texenv("foo", "bar")), begin("foo", "bar"))
})
